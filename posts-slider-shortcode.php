<?php
	/*
	Plugin Name: Posts Slider Shortcode
	Plugin URI: http://wordpress.org/plugins/posts-slider-shortcode
	Description: Wordpress Posts Slider is a WordPress plugin Slider of default posts with responsive design.
	Version: 1.0
...
define('ELSPOST_SLIDER_PLUGIN_PATH', WP_PLUGIN_URL . '/' . plugin_basename( dirname(__FILE__) ) . '/' );
define('elspost_slider_plugin_dir', plugin_dir_path( __FILE__ ) );


function elspost_slider_load_textdomain(){
	load_plugin_textdomain('elspostslider', false, dirname(plugin_basename( __FILE__ )) . '/languages/');
}
add_action('plugins_loaded', 'elspost_slider_load_textdomain');




function elspost_slider_script_initial(){
 wp_enqueue_style('elspost-slider-transitions-css', ELSPOST_SLIDER_PLUGIN_PATH.'css/owl.transitions.css');
 wp_enqueue_style('elspost-slider-slider-css', ELSPOST_SLIDER_PLUGIN_PATH.'css/owl.carousel.css');
 wp_enqueue_style('elspost-slider-awesome-css', ELSPOST_SLIDER_PLUGIN_PATH.'css/font-awesome.css');
 wp_enqueue_style('elspost-slider-theme-css', ELSPOST_SLIDER_PLUGIN_PATH.'css/owl.theme.css');
 wp_enqueue_script('elspost-slider-slider-js', plugins_url( '/js/owl.carousel.js', __FILE__ ), array('jquery'), '1.0', false);
 }
add_action('wp_enqueue_scripts', 'elspost_slider_script_initial');
...
function elspost_slider_register_shortcode($atts, $content = null){
		...
		if($category > -1) {
			...
			$elspostslider_query = new WP_Query( $args );
				...
				if($post_styles=="style1")
					...
					<script type="text/javascript">
						jQuery(document).ready(function($) {
							$("#elspost-main-slider-'.$psrndn.'").owlCarousel({
							autoPlay: '.$auto_play.',
							stopOnHover: true,
							items : '.$show_items.',
							itemsDesktop : [1199,3],
					...
					$result.='<div class="elspost-slider-area'.$psrndn.'">';
					$result.='<div id="elspost-main-slider-'.$psrndn.'" class="owl-carousel">';
					// Creating a new side loop
					while ( $elspostslider_query->have_posts() ) : $elspostslider_query->the_post();
...
add_shortcode('elspostslider', 'elspost_slider_register_shortcode');
